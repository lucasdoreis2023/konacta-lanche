...
import { Loader2, Receipt, Users, DollarSign, CheckCircle2, Key, QrCode, Copy } from 'lucide-react';
...
export default function AdminTables() {
  ...
  const copyTableLink = (tableNum: number) => {
    const url = `${window.location.origin}/mesa/${tableNum}`;
    navigator.clipboard.writeText(url);
    toast.success(`Link da mesa ${tableNum} copiado!`);
  };
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Acompanhe as mesas ativas e gerencie senhas
            </p>
          </div>
        ...
        <Tabs defaultValue="active">
          <TabsList>
            <TabsTrigger value="active">Mesas Ativas</TabsTrigger>
            ...
            {isLoading ? (
              ...
              <Card>
                <CardContent className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <p className="text-muted-foreground">
                    Quando clientes ativarem uma mesa e fizerem pedidos, eles aparecerão aqui.
                  </p>
                </CardContent>
              </Card>
            ) : (
          ...
          <TabsContent value="all" className="mt-4">
            {tablesLoading ? (
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {tables?.map(table => {
                  const isOccupied = occupiedTableNumbers.has(table.table_number);
                  return (
                    <Card key={table.id} className={!table.active ? 'opacity-50' : ''}>
                      <CardContent className="p-4 space-y-3">
                        ...
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          <span className="font-mono tracking-widest">{table.password}</span>
                        </div>
                        <div className="flex gap-2">
                          <Button
                            ...
                            onClick={() => copyTableLink(table.table_number)}
                          >
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={() => {
                              navigator.clipboard.writeText(table.password);
